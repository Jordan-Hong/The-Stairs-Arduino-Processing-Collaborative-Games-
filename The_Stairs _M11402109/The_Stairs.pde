...
void refreshMovementState() {
  if (gameState == STATE_PLAYING) {
    leftPressed = (hwXState == -1);
    rightPressed = (hwXState == 1);
  } else {
    leftPressed = false;
    rightPressed = false;
  }
...
void drawLevel1IntroScreen() {
  ...
  textFont(uiFont, 17);
  text("Move: <-- / -->", SCREEN_W / 2, 295);
  text("Dash: SW", SCREEN_W / 2, 315);

  float tw = textWidth("Press SW to Start.");
  rectMode(CENTER);
  noStroke();
  ...
  fill(30);
  text("Press SW to Start.", SCREEN_W / 2, 390);
}
...
void drawLevel2IntroScreen() {
  ...
  text("Pick Up Hearts: Piezo", SCREEN_W / 2, 323);
...
void drawLevel3IntroScreen() {
  ...
  text("Shield: Cover Photoresistor For 2 Sec.", SCREEN_W / 2, 330);
  text("Pick Up Items: Piezo", SCREEN_W / 2, 352);
  ...
  fill(30);
  text("Press SW to Start.", SCREEN_W / 2, 450);
}
...
void drawLevel2HUD() {
  ...
  textAlign(LEFT, CENTER);
  textFont(uiFont, 14);
  text("Level 2: Touch Piezo to pick up hearts", 20, 79);
}
...
void drawLevel3HUD() {
  ...
  if (shieldActive) {
    ...
    if (cd > 0) {
      ...
      text("Cover photoresistor for 2 sec to shield", 20, 124);
    ...
    text("Piezo to pick up hearts / stars", 20, 146);
  }
}
