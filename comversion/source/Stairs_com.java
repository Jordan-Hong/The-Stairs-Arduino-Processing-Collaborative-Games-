...
public class Stairs_com extends PApplet {
...
// 世界 / 階梯參數（第四版）
// =====================================================
final float LEVEL1_SCROLL_SPEED = 1.5f;
final float LEVEL2_SCROLL_SPEED = 1.6f;
final float LEVEL3_SCROLL_SPEED = 1.8f;
...
// Level 1
final float L1_SPIKE_RATIO = 0.40f;

// Level 2 / Level 3
final float L23_SPIKE_RATIO = 0.25f;
final float L23_SLIDE_RATIO = 0.28f;

// Level 2 / 3 平滑階梯中的愛心比例
final float HEART_ON_SMOOTH_RATIO = 0.35f;
final float STAR_ON_SMOOTH_RATIO = 0.25f;

// =====================================================
// 火球參數（Level 3）
// =====================================================
final float FIREBALL_SIZE = 34;
final float FIREBALL_SPEED = 4.6f;
...
public void updateViewTransform() {
  viewScale = min(width / PApplet.parseFloat(SCREEN_W), height / PApplet.parseFloat(SCREEN_H));
  viewOffsetX = (width - SCREEN_W * viewScale) * 0.5f;
  viewOffsetY = (height - SCREEN_H * viewScale) * 0.5f;
}
...
public void updateBootWarmup() {
  ...
  bootProgress = 0.70f + 0.28f * (doneFrames / PApplet.parseFloat(totalWarmupFrames));
...
public void drawLoadingScreen() {
  ...
  noStroke();
  for (int i = 0; i < 24; i++) {
    float sx = (i * 97 + frameCount * 0.6f) % SCREEN_W;
  ...
  float runnerX = SCREEN_W / 2 + sin(loadingAnimT * 1.4f) * 90;
  float runnerY = 240 + sin(loadingAnimT * 2.2f) * 8;
  ...
  fill(245);
  textFont(uiFont, 16);
  text(PApplet.parseInt(constrain(bootProgress, 0, 1) * 100) + "%", SCREEN_W / 2, 405);
...
public void drawLoadingStairs(float x, float y) {
  pushStyle();
  rectMode(CORNER);
  noStroke();
...
public void drawLoadingRunner(float x, float y) {
  pushStyle();
  noStroke();
...
public PImage tryLoadImageFromData(String relativePath) {
  if (!fileExistsInData(relativePath)) return null;
  return loadImage(relativePath);
}

public SoundFile tryLoadSound(String relativePath) {
...
public void drawFullScreenImage(PImage img) {
  if (img == null) return;
  imageMode(CORNER);
  image(img, 0, 0, SCREEN_W, SCREEN_H);
}

public void playOneShot(SoundFile sf) {
  if (sf == null) return;
  sf.stop();
  sf.play();
}

public void stopSound(SoundFile sf) {
  if (sf == null) return;
  sf.stop();
}

public void switchBgm(SoundFile nextBgm) {
...
public void applyScreenAudioForState() {
...
public void goToLevelIntro(int level) {
  currentLevel = level;
...
public void queueTransition(int action, int level, boolean withClick) {
...
public void clearTransitionLock() {
...
public int getSoundDurationMs(SoundFile sf) {
  if (sf == null) return 0;
  return max(1, PApplet.parseInt(sf.duration() * 1000));
}

public void executeQueuedTransition() {
...
public void updateDeferredActions() {
...
public void startDeathSequence(boolean playHitFirst) {
  switchBgm(null);
  pendingVictorySequence = false;
  pendingVictoryNextTime = 0;
  pendingVictoryAction = ACTION_NONE;
  ...
  if (playHitFirst) {
    playOneShot(sfxHit);
    if (sfxHit != null) {
      pendingDeathNextTime = millis() + max(1, PApplet.parseInt(sfxHit.duration() * 1000));
    } else {
...
public void updatePendingDeathSequence() {
  if (!pendingDeathSequence) return;

  if (pendingDeathStage == 0 && millis() >= pendingDeathNextTime) {
    playOneShot(sfxDeath1);
    if (sfxDeath1 != null) {
      pendingDeathNextTime = millis() + max(1, PApplet.parseInt(sfxDeath1.duration() * 1000));
    } else {
      pendingDeathNextTime = millis();
    }
    pendingDeathStage = 1;
  ...
  if (pendingDeathStage == 1 && millis() >= pendingDeathNextTime) {
    ...
    if (sfxDeath2 != null) {
      pendingDeathNextTime = millis() + max(1, PApplet.parseInt(sfxDeath2.duration() * 1000));
    } else {
      pendingDeathNextTime = millis();
    }
    pendingDeathStage = 2;
...
public void startVictorySequence(int nextAction, int nextLevel, boolean withGoldAura) {
...
public void loadAllPlayerSprites() {
...
public PImage[] loadSequentialFramesFlexible(String subfolder, String baseName, int startIdx, int endIdx, boolean allowRoot) {
...
public PImage tryLoadSprite(String subfolder, String filename) {
  String relativePath;
...
public boolean fileExistsInData(String relativePath) {
  File f = new File(dataPath(relativePath));
  return f.exists() && f.isFile();
}

public int loopFrameIndex(int totalFrames, int hold) {
...
public void handleMenuMove(int dir) {
  if (transitionLocked) return;
...
public void handleKeyboardConfirmOrPickupInput() {
  if (transitionLocked) return;
...
public void handleKeyboardDashInput() {
  if (transitionLocked) return;
...
public void handleDashOrSelectInput() {
  if (transitionLocked) return;
...
public float getWorldScrollSpeed() {
...
public void drawLevel1IntroScreen() {
...
public void drawLevel2IntroScreen() {
...
public void drawLevel3IntroScreen() {
...
public void drawMenuOption(String label, float x, float y, boolean selected, int normalTextColor) {
  pushStyle();
  textAlign(CENTER, CENTER);
...
public void drawLevel3HUD() {
  ...
  if (shieldActive) {
    ...
    if (cd > 0) {
      text("Shield cooldown: " + nf(cd / 1000.0f, 1, 1) + " s", 20, 124);
    } else {
      text("Hold X for 2 sec to shield", 20, 124);

      if (shieldSenseNow() && shieldSenseStartTime != -1) {
        float holdSec = (millis() - shieldSenseStartTime) / 1000.0f;
        holdSec = min(holdSec, 2.0f);
...
public void drawHeart(float x, float y, float size, int c) {
  pushMatrix();
  pushStyle();
  translate(x, y);
  noStroke();
  fill(c);
  ellipse(-size * 0.35f, -size * 0.20f, size * 0.75f, size * 0.75f);
  ellipse(size * 0.35f, -size * 0.20f, size * 0.75f, size * 0.75f);
  triangle(-size * 0.80f, 0, size * 0.80f, 0, 0, size * 1.1f);
  popStyle();
  popMatrix();
}


public void drawStar(float x, float y, float rOuter, int fillColor, int strokeColor) {
  pushMatrix();
  pushStyle();
  translate(x, y);
  fill(fillColor);
  ...
  for (int i = 0; i < 10; i++) {
    ...
    float r = (i % 2 == 0) ? rOuter : rOuter * 0.45f;
...
public void updateFireballSpawner() {
...
public void spawnStairRow(float y) {
  int num = (random(1) < 0.35f) ? 2 : 1;
  ...
  if (num == 2) {
    ...
    int safeGuard = 0;
    while (abs(x2 - s1.x) < STAIR_W * 0.75f && safeGuard < 20) {
...
public Stair createStairForCurrentLevel(float x, float y, boolean allowSpikeThisRow) {
  if (currentLevel == 1) {
    int type;
    ...
    if (allowSpikeThisRow) {
      ...
      float nonSpikeSlideRatio = L23_SLIDE_RATIO / (1.0f - L23_SPIKE_RATIO);
      float r = random(1);
...
public void handlePlayerStairCollision() {
...
public void applyAttackDamage(int hazardType) {
...
public void tryPickCollectibles() {
...
public boolean rectOverlap(float x1, float y1, float w1, float h1,
...
class Player {
  ...
  public void startKoAnim() {
    specialAnim = SPECIAL_KO;
    specialAnimStartFrame = frameCount;
  }

  public boolean isKoAnimFinished() {
  ...
  public void display() {
   /* boolean visible = true;
    ...
    // 4) RUN
    if (currentImg == null && onStair && abs(lastMoveX) > 0.1f && runFrameCount > 0) {
...
class Fireball {
  ...
  public void display() {
    ...
    fill(255, 150, 40, 220);
    ellipse(x + size / 2, y + size / 2, size * 0.65f, size * 0.65f);

    fill(255, 230, 120, 200);
    ellipse(x + size / 2, y + size / 2, size * 0.30f, size * 0.30f);
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Stairs_com" };
    if (passedArgs != null) {
